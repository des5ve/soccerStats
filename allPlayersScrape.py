...
leagueURLs = [prem, laLiga, ligueOne, bundesliga, serieA]
...
for i in leagueURLs:
    players = pd.read_html(i, encoding='utf8')[0] 
...
bundesligaFrame = leagues("bundesliga")
laLigaFrame = leagues("laLiga")
ligueOneFrame = leagues("ligueOne")
serieAFrame = leagues("serieA")
frames = [premFrame, bundesligaFrame, ligueOneFrame, serieAFrame, laLigaFrame]
result = pd.concat(frames)



result.to_csv('AllPlayers.csv', index = False)