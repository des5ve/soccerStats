...
#Function that returns dataframe of top goalscorers in the league.
def topGoalScorers(league, goals):
    if league == "epl":
        url = prem
...
#Put the below in the above
def cleanFrame(dataframe):
    dataframe.columns = dataframe.columns.droplevel(0)
    dataframe.columns = ['Rk', 'Player', 'Nation', 'Pos', 'Squad', 'Age', 'Born', 'MP', 'Starts', 'Min', 'Gls', 'Ast', 'PK', 'PKatt', 'CrdY', 'CrdR', 'Gls/90','Ast/90','G+A/90','G-PK/90', 'G+A-PK/90', 'xG', 'npxG', 'xA', 'xG/90', 'xA/90', 'xG+xA/90', 'npxG/90', 'npxG+xA/90', 'Matches']
    removal = dataframe[dataframe.xG != 'xG']
    playersClean = removal.astype(convert_dict)
    return playersClean



eplScorers = topGoalScorers("epl", 5)
print (eplScorers)